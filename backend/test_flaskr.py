...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_create_question(self):
        ...
        question = Question.query.filter_by(
            id=response_data['created']).one_or_none()
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response_data['success'], True)
        self.assertEqual(
            response_data['question_created'],
    ...
    def test_search_questions_invalid_404(self):
        response = self.client().post(
            '/questions', json={'searchTerm': 'iiiiiiiiiiiii'})
        response_data = json.loads(response.data)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response_data['success'], False)
        self.assertEqual(response_data['message'], 'resource not found')
    ...
    def test_get_questions_by_category(self):
        ...
        response = self.client().get(
            '/categories/{}/questions'.format(category_id))
        response_data = json.loads(response.data)
        ...
        self.assertEqual(response_data['success'], True)
        self.assertEqual(
            len(response_data['questions']), len(category_questions))
    ...
    def test_retrieve_quizzes_invalid_category_400(self):
        response = self.client().post(
            '/quizzes', json=self.quiz_invalid_category)
        response_data = json.loads(response.data)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response_data['success'], False)
        self.assertEqual(response_data['message'], 'bad request')
    ...
    def test_retrieve_quizzes_invalid_request_400(self):
        response = self.client().post(
            '/quizzes', json=self.quiz_invalid_request)
        response_data = json.loads(response.data)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response_data['success'], False)
        self.assertEqual(response_data['message'], 'bad request')
